...
- **P5 Enron qrels + topic 201 review** — idmap cascade vs TREC 2010 qrels (coverage gate ≥ ~90% of judged docs for selected custodians **before any token spend**; else swap custodians — idmap per-custodian is cheap); strata-aware metrics; dev set → full run, reusing P2 engine unchanged. **Open decision (recorded 2026-07-21, groundwork numbers added 2026-07-22 — see `artifacts/enron_idmap_coverage.json`):** the qrels judge attachment PARTS as separate docids (`<parent>.N`; ~46% of topic-201 judged docs, ~55% of relevant) while the ingest idmap is message-level. Groundwork findings that reframe the choice: (a) topic 201's relevant judged docs belong to Enron LEGAL staff — shackleton-s 147, jones-t 64, griffith-j 51; skilling-j/lay-k hold ZERO, kaminski-v 10 — so the 3-custodian fold policy cannot support recall estimation for 201 regardless of part policy; (b) the official Learning-task participant text distribution (`trec/edrmv2txt-v2.tar.bz2`, sha-pinned) contains text for 100% of the 21,760 judged docs (messages AND parts, all 151 custodians), making Learning-task-faithful scoring of the full judged sample possible at trivial cost — parts reviewed as their own documents per the task's official design, no fold needed for scoring; (c) the parser + weighted metrics reproduce the official estimator exactly (calc1.c R for 200/204/207; end-to-end perfect-classifier check for 201). Dan's decision: scoring universe (judged-sample-from-official-text vs custodian-scoped) and, if custodian-scoped work should intersect 201, whether to add/swap toward shackleton-s/jones-t/griffith-j (which would also enrich P6 — Shackleton is in-house ENA counsel).
