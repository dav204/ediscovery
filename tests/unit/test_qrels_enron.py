...
def test_full_custodian_map_builds_and_caches(tmp_path):
    ...
    assert mapping == {"3.11.AAA": "shackleton-s", "3.11.AAA.1": "shackleton-s",
                       "3.22.BBB": "kaminski-v"}
    tarball.unlink()  # cache hit must not re-read the tarball
    assert full_custodian_map(tarball, cache) == mapping
